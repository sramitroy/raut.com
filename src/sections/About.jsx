...
const VALUES = [
  { name: 'Discipline', line: 'We underwrite conservatively and say no often.' },
  { name: 'Stewardship', line: 'Every asset is cared for as if we will hold it forever.' },
  { name: 'Ingenuity', line: 'We build our own tools when the market falls short.' },
  { name: 'Integrity', line: 'We do what we say — with partners, tenants, and teams.' },
]
  ...
  return (
    <section id="about" className="relative bg-paper py-24 sm:py-32">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <Reveal>
          <Eyebrow>About Raut</Eyebrow>
        </Reveal>

        <div className="mt-10 grid grid-cols-1 gap-8 lg:grid-cols-5">
          <Reveal className="lg:col-span-3">
            <div className="h-full rounded-2xl bg-blue p-8 text-paper sm:p-12">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-paper/70">
                Our Mission
              </p>
              <p className="mt-6 font-display text-2xl font-bold leading-snug tracking-tight sm:text-4xl">
                To compound value patiently — acquiring durable assets, operating them with
                care, and building the technology that makes both better.
              </p>
            </div>
          </Reveal>

          <Reveal delay={0.1} className="lg:col-span-2">
            <div className="h-full rounded-2xl border border-ink/10 bg-white/60 p-8 sm:p-12">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-ink/50">
                Where We Operate
              </p>
              <ul className="mt-6 space-y-4">
                {['Ohio', 'New Jersey', 'Pennsylvania'].map((m) => (
                  <li
                    key={m}
                    className="flex items-center gap-4 border-b border-ink/10 pb-4 font-display text-2xl font-bold tracking-tight text-ink"
                  >
        ...
        <div className="mt-8 grid grid-cols-1 gap-px overflow-hidden rounded-2xl bg-ink/10 sm:grid-cols-2 lg:grid-cols-4">
          {VALUES.map((v, i) => (
            <Reveal key={v.name} delay={i * 0.08} className="bg-paper p-8">
              <p className="font-display text-xl font-bold tracking-tight text-ink">{v.name}</p>
              <p className="mt-3 text-sm leading-relaxed text-ink/60">{v.line}</p>
            </Reveal>
          ))}
        </div>

        {/* Founder */}
        <div id="founder" className="mt-20 grid grid-cols-1 gap-10 lg:grid-cols-5">
